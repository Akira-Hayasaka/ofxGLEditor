...
void SyntaxHighlight::setup() {
	...
	colorScheme.setPreprocessorColor(ofColor(230,121,57));
	colorScheme.setTypenameColor(ofColor(246,70,185));
	colorScheme.setFunctionColor(ofColor(11,179,244));
	
	syntax.setSingleLineComment("//");
    syntax.setMultiLineComment("/*", "*/");
}
...
void SyntaxHighlight::setDataTypes() {
	...
	syntax.setWords(dataType, ofxEditorSyntax::TYPENAME);
}
...
void SyntaxHighlight::setFunctions() {
	...
	syntax.setWords(functions, ofxEditorSyntax::FUNCTION);
}